  ...
  stages{
       stage('hosting application'){
        steps{
          sh "ls"
          script{
              def cmd = "aws rds create-db-instance --db-instance-identifier test-mysql-instance6 --db-name cpms --db-instance-class db.t2.micro --vpc-security-group-ids "+SecurityGroup+" --engine mysql --engine-version 5.7 --db-parameter-group-name default.mysql5.7 --publicly-accessible  --master-username admin --master-user-password ramana4u2021 --allocated-storage 10 --region us-east-2"
