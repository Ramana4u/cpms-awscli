pipeline{
  ...
  stages{
       stage('LB'){
        steps{
            sh "aws elb create-load-balancer --load-balancer-name my-load-balancer --subnets "+Subnet+"  --security-groups " + SecurityGroup + " --region us-east-2"
            script{
            ...
            sh "aws elbv2 create-listener --load-balancer-arn loadbalancer-arn --protocol HTTP --port 80 --default-actions Type=forward,TargetGroupArn=${jsonitem['TargetGroups'][0]['TargetGroupArn']} --region us-east-2"
